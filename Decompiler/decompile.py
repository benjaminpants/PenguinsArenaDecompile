...
def decompile(dso, sink=None, in_function=False, offset=0):
    ...
    while ip < len(dso.code):
        ...
        if opcode == "OP_DOCBLOCK_STR":
            ...
            if string_stack[-1][-1] == "[":
                print(indentation*"\t" + '%s = %s%s];' % (current_variable, string_stack.pop(), string_stack.pop()), file=sink)
            else:
            ...
            if current_field[-1] == "[":
                float_stack.append("%s.%s%s]" % (current_object, current_field, string_stack.pop()))
            else:
                float_stack.append("%s.%s" % (current_object, current_field))
        elif opcode == "OP_LOADFIELD_STR":
            if current_field[-1] == "[":
                string_stack.append("%s.%s%s]" % (current_object, current_field, string_stack.pop()))
            else:
                string_stack.append("%s.%s" % (current_object, current_field))
        elif opcode == "OP_LOADFIELD_UINT":
            if current_field[-1] == "[":
                int_stack.append("%s.%s%s]" % (current_object, current_field, string_stack.pop()))
            else:
            ...
            if current_object is None:  # This is an object creation
                if current_field[-1] == "[":
                    int_stack[-1] += indentation*"\t" + "%s%s] = %s;\n" % (current_field, string_stack.pop(), string_stack.pop())
                elif is_number(string_stack[-1]) or string_stack[-1] == "\"\"" or string_stack[-1][0] == "%" or string_stack[-1][0] == "\"":
                    int_stack[-1] += indentation*"\t" + "%s = %s;\n" % (current_field, string_stack.pop())
                else:
                    int_stack[-1] += indentation*"\t" + "%s = \"%s\";\n" % (current_field, string_stack.pop())
                    
            else:  # This is a field affectation
                if current_field[-1] == "[":
                    print(indentation*"\t" + "%s.%s%s] = %s;" % (current_object, current_field, string_stack.pop(), string_stack.pop()), file=sink)
                else:
                    print(indentation*"\t" + "%s.%s = %s;" % (current_object, current_field, string_stack[-1]), file=sink)
        elif opcode == "OP_SAVEFIELD_FLT":
            if current_object is None:  # This is an object creation
                if current_field[-1] == "[":
                    int_stack[-1] += indentation*"\t" + "%s%s] = %s;\n" % (current_field, float_stack.pop(), string_stack.pop())
                else:
